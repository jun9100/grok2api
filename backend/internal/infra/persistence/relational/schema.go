...
var schemaModels = []any{
	...
	&egressIPRecordModel{},
	&egressIPLeaseModel{},
	&buildAccountRiskObservationModel{},
	&buildEgressUsageWindowModel{},
	&buildEgressOutcomeWindowModel{},
	&egressNodeModel{},
...
var schemaIndexes = []string{
	...
	"CREATE UNIQUE INDEX IF NOT EXISTS uidx_egress_ip_records_scope_family_exit_ip ON egress_ip_records(scope, family, exit_ip)",
	"CREATE INDEX IF NOT EXISTS idx_egress_ip_records_scope_last_seen ON egress_ip_records(scope, last_seen_at DESC, id DESC)",
	"CREATE INDEX IF NOT EXISTS idx_egress_ip_records_node_seen ON egress_ip_records(last_node_id, last_seen_at DESC, id DESC)",
	"CREATE UNIQUE INDEX IF NOT EXISTS uidx_egress_ip_leases_active_account_scope ON egress_ip_leases(account_id, scope) WHERE state = 'active'",
	"CREATE INDEX IF NOT EXISTS idx_egress_ip_leases_record_state_expiry ON egress_ip_leases(ip_record_id, state, expires_at)",
	"CREATE INDEX IF NOT EXISTS idx_egress_ip_leases_account_scope ON egress_ip_leases(account_id, scope, id DESC)",
