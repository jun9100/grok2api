...
func TestAuditRepositoryBatchAndCursor(t *testing.T) {
	...
	values := []audit.Record{
		...
		{RequestID: "cursor-3", ClientKeyID: 1, ClientKeyName: "production", ModelRouteID: 1, ModelPublicID: "grok-test", ModelUpstreamModel: "grok-test-upstream", AccountID: uint64Pointer(43), AccountName: "primary", EgressNodeID: uint64Pointer(42), EgressIPRecordID: uint64Pointer(44), EgressNodeName: "proxy-shanghai", EgressScope: "grok_build", EgressMode: audit.EgressModeProxy, BuildBotFlagSource: 2, StatusCode: 200, CreatedAt: now.Add(-time.Minute)},
	}
	...
	if first[0].ClientKeyName != "production" || first[0].ModelPublicID != "grok-test" || first[0].ModelUpstreamModel != "grok-test-upstream" || first[0].AccountID == nil || *first[0].AccountID != 43 || first[0].AccountName != "primary" || first[0].EgressNodeID == nil || *first[0].EgressNodeID != 42 || first[0].EgressIPRecordID == nil || *first[0].EgressIPRecordID != 44 || first[0].EgressNodeName != "proxy-shanghai" || first[0].EgressMode != audit.EgressModeProxy || first[0].BuildBotFlagSource != 2 {
